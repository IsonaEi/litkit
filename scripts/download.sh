#!/usr/bin/env bash
# Download a paper PDF by DOI or URL, trying multiple legal sources in order:
#   direct URL → publisher PDF → EuropePMC → arXiv.
#
# Usage: download.sh [--enable-scihub] <DOI_or_URL> <output_dir/> [filename]
# Exit 0 on success, 1 on failure (needs manual download).
#
# Sci-Hub fallback is OFF by default. It is only attempted when EITHER the
# --enable-scihub flag is passed OR the environment variable LITKIT_ENABLE_SCIHUB=1
# is set. Enabling it is your responsibility — see the disclaimer printed at use.

set -euo pipefail

# --- Parse the optional --enable-scihub flag from anywhere in the args ---
ENABLE_SCIHUB="${LITKIT_ENABLE_SCIHUB:-0}"
POSITIONAL=()
for arg in "$@"; do
  if [[ "$arg" == "--enable-scihub" ]]; then
    ENABLE_SCIHUB=1
  else
    POSITIONAL+=("$arg")
  fi
...
if [[ $# -lt 1 ]]; then
  echo "Usage: download.sh [--enable-scihub] <DOI_or_URL> <output_dir/> [filename]" >&2
  exit 1
fi
...
# Strategy 5: Sci-Hub (opt-in only)
# This fallback is disabled by default. The legal default sources above
# (publisher / EuropePMC / arXiv) are the recommended way to obtain papers.
if [[ -n "$DOI" && "$ENABLE_SCIHUB" == "1" ]]; then
  {
    echo "⚠️  Sci-Hub fallback enabled (--enable-scihub / LITKIT_ENABLE_SCIHUB=1)."
    echo "    Accessing papers via Sci-Hub may violate publisher terms of service or"
    echo "    copyright law in your jurisdiction. You are solely responsible for ensuring"
    echo "    your use is legal and ethical. The recommended sources are the publisher,"
    echo "    EuropePMC, and arXiv, which are tried by default above."
  } >&2
...
if [[ "$ENABLE_SCIHUB" != "1" && -n "$DOI" ]]; then
  echo "   (Sci-Hub fallback is disabled; re-run with --enable-scihub to opt in.)"
fi
exit 1
